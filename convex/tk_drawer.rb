...
module TkDrawer
  ...
  # рисование линии
  def TkDrawer.draw_line(p,q,color = "black")
    TkcLine.new(CANVAS, x(p), y(p), x(q), y(q)) {fill(color)}
  end

  def TkDrawer.draw_rect(p,q)
    TkcRectangle.new(CANVAS, x(p), y(p),
     x(q), y(q), "width"=>1)
  end
...
class Figure
  def draw
    TkDrawer.clean
    TkDrawer.draw_rect(@@rect[0], @@rect[1])
  end
end
class Point < Figure
  def draw

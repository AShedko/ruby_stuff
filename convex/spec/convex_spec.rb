...
describe Segment do
  ...
  it "Двуугольник имеет 2 острых угла" do
    expect(fig.angles).to eql 2
  end
...
describe Polygon do
  ...
  context "Угловые свойства многоугольника" do
    it "У прямоугольного треугольника 2 острых угла" do
      expect(fig.angles).to eql 2
    end

    it "У остроугольного треугольника 3 острых угла" do
      expect(fig.add(R2Point.new(-1,-1)).angles).to eql 3
    end

    it "У квадрата 0 острых углов" do
      fig.add(R2Point.new(1,1))
      expect(fig.angles).to eql 0
    end

    it "У повёрнутого квадрата также 0 острых углов" do
      fig.add(R2Point.new(-1,0))
      fig.add(R2Point.new(0,-1))
      expect(fig.angles).to eql 0
    end


    it "Изменения выпуклой оболочки могут изменять число острых углов" do
      ...
      expect(fig.angles).to eql 1
    end

    it "Ромб"do
      fig.add(R2Point.new(-1,0))
      fig.add(R2Point.new(0,3))
      fig.add(R2Point.new(0,-3))
      expect(fig.angles).to eql 2
    end
  end
